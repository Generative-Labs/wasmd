...
HTTPS_GIT := https://github.com/Generative-Labs/wasmd.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=wasm \
		  ...
		  -X github.com/Generative-Labs/wasmd/app.Bech32Prefix=wasm \
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "./tests/system/vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs gci write --skip-generated -s standard -s default -s "prefix(cosmossdk.io)" -s "prefix(github.com/cosmos/cosmos-sdk)" -s "prefix(github.com/Generative-Labs/wasmd)" --custom-order
